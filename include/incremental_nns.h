    ...
    class incremental_nns {
        public:
            ...
            incremental_nns(
                ...
                const size_t k_inner);

            // Merges i, j to a single node with new_id and return neighbours of this single node and their associated edge costs.
            std::vector<std::tuple<size_t, size_t, REAL>> merge_nodes(const size_t i, const size_t j, const size_t new_id, const feature_index<REAL>& index, const bool do_exhaustive_search);
            ...
            void print(const feature_index<REAL>&index);
        private:
            ...
            std::vector<std::vector<size_t>> nn_inverse_graph_;
            size_t k_, k_inner_;
