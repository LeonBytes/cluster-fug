...
PYBIND11_MODULE(dense_multicut_py, m) {
    ...
    m.def("dense_gaec_incremental_nn", [](std::vector<double> features, const int num_nodes, int dim, const float dist_offset, const size_t k_inc_nn, const std::string solver_type, const std::string index_type) 
    {
        ...
        if (solver_type ==  "adj_matrix")
            labeling = dense_gaec_adj_matrix<double>(num_nodes, dim, features, track_dist_offset);
        else if (solver_type ==  "flat_index")
            labeling = dense_gaec_flat_index<double>(num_nodes, dim, features, track_dist_offset);
        else if (solver_type ==  "hnsw")
            labeling = dense_gaec_hnsw<double>(num_nodes, dim, features, track_dist_offset);
        else if (solver_type ==  "parallel_flat_index")
            labeling = dense_gaec_parallel_flat_index<double>(num_nodes, dim, features, track_dist_offset);
        else if (solver_type ==  "parallel_hnsw")
            labeling = dense_gaec_parallel_hnsw<double>(num_nodes, dim, features, track_dist_offset);
        else if (solver_type ==  "flat_index")
            labeling = dense_gaec_flat_index<double>(num_nodes, dim, features, track_dist_offset);
        else if (solver_type ==  "inc_nn_flat")
            labeling = dense_gaec_incremental_nn<double>(num_nodes, dim, features, k_inc_nn, "Flat", track_dist_offset);
        else if (solver_type ==  "inc_nn_hnsw")
            labeling = dense_gaec_incremental_nn<double>(num_nodes, dim, features, k_inc_nn, "HNSW64", track_dist_offset);
        else
